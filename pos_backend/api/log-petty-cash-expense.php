...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['branchId'])) {
    ...
    // Add the expense to the main cash box log for tracking
    $id = 'cbe_' . time();
    $sql = "INSERT INTO cash_box_entries (id, branchId, date, type, category, description, amountLBP, amountUSD, isManual) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    safe_query($conn, $sql, [$id, $branchId, date('Y-m-d H:i:s'), 'expense', $data['category'], 'Petty Cash: ' . $data['description'], $data['amountLBP'], $data['amountUSD'], 0]);
